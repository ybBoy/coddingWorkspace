<template>
  <div class="page-container">
    <div class="page-title">
      <i class="el-icon-s-order" style="margin-right: 10px;"></i>提交出库申请
    </div>
            ...
            <el-table-column label="申请数量" width="180">
              <template slot-scope="scope">
                <el-input-number
                  ...
                  :min="1"
                  :max="getMaxQuantity(scope.row.partId)"
      ...
      >
        <template slot="default">
          <ul style="margin: 10px 0; padding-left: 20px;">
            <li>请选择需要出库的零件和数量，系统会根据当前库存自动限制最大申请数量</li>
            <li>申请提交后，状态为"待审核"，等待管理员审核</li>
            <li>管理员审核通过后，系统会自动完成出库操作</li>
  ...
  methods: {
    async loadParts() {
      ...
      try {
        ...
        if (res.success) {
          this.availableParts = (res.data || []).filter(p => p.quantity > 0)
        } else {
    ...
    getMaxQuantity(partId) {
      const part = this.getPartInfo(partId)
      return part ? part.quantity : 99999
    },
    ...
    validateForm() {
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        if (item.quantity < 1) {
          this.$message.warning('第 ' + (i + 1) + ' 行申请数量必须大于0')
          return false
        }
        const part = this.getPartInfo(item.partId)
        if (part && item.quantity > part.quantity) {
          this.$message.warning('零件 "' + part.name + '" 申请数量不能超过当前库存 (' + part.quantity + ')')
          return false
        }
      }
    ...
    async submitRequest() {
      ...
      try {
        const res = await partApi.user.submitRequest(submitData)
        if (res.success) {
          this.$message.success('申请提交成功！申请编号: ' + res.data.id)
