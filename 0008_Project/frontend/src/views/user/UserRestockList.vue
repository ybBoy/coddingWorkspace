<template>
  <div class="page-container">
    ...
    <el-empty v-if="!loading && parts.length === 0" description="太棒了！所有零件库存正常">
      <el-button type="primary" icon="el-icon-s-grid" @click="$router.push('/user/search')">搜索零件</el-button>
    </el-empty>
...
export default {
  ...
  methods: {
    async loadParts() {
      this.loading = true
      try {
        const res = await partApi.user.getVisiblePartsNeedRestock()
        if (res.success) {
          this.parts = res.data || []
        } else {
    ...
    async confirmBatchStockIn() {
      ...
      for (const part of this.selectedParts) {
        try {
          const res = await partApi.user.stockIn(part.id, this.stockInQuantity)
          if (res.success) {
            successCount++
          } else {
            failCount++
    ...
    async confirmSingleStockIn() {
      try {
        const res = await partApi.user.stockIn(this.currentPart.id, this.singleStockInQuantity)
        if (res.success) {
