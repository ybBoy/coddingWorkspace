...
2. 对过号号码可以执行 3 种操作：
   - **重新入队**：放回等待队列末尾
   - **直接重叫**：立即在指定窗口叫号
   - **结束**：标记为完成，并加入叫号历史记录（finished 类型）
...
2. **Toast 操作反馈**：所有操作成功/失败都有 Toast 提示，自动滑入滑出，仅操作发起端显示
3. **过号管理**：独立过号列表，支持重新入队、直接重叫、标记结束 3 种操作
4. **业务类型筛选**：叫号时可指定业务类型，仅从对应类型队列中按时间取号
5. **窗口配置**：支持动态新增、编辑、启用/停用窗口，配置支持的业务类型
6. **今日统计**：6 项指标实时更新，含平均等待时长计算，按日期自动过滤
7. **WebSocket 增强**：支持 VITE_WS_URL 配置，ws/wss 自动选择，待发送队列，开发环境使用当前 host（而非写死 localhost，支持 iPad/局域网访问）
