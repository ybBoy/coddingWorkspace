<template>
  <div class="page-container">
    ...
    <el-table :data="records" v-loading="loading" style="width: 100%; margin-top: 20px;">
      ...
      <el-table-column prop="ipAddress" label="操作IP" width="130">
        <template slot-scope="scope">
          <el-tag size="mini" type="warning">{{ scope.row.ipAddress || '未知' }}</el-tag>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    async loadSummary() {
      try {
        const res = await partApi.user.getRecordsSummary()
        if (res.success) {
    ...
    async loadRecords() {
      ...
      try {
        ...
        const res = await partApi.user.getRecords(params)
        if (res.success) {
